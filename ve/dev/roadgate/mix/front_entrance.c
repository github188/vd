...
static void front_set_up_signal(void)
{
	rg_set_up_signal();
}

static void front_do_dummy(void)
{
	return;
}
 ...
 * @return int_fast32_t
 */
static int_fast32_t front_idle(rg_t *rg)
{
	...
	if (ctrl->ctrler == RG_CTRLER_LO) {
		...
		static const ctrl_handle_t tab[RG_ACTION_END] = {
			[RG_ACTION_UP] = {front_set_up_signal, STATUS_LO_OPEN}
		};
...
static int_fast32_t front_lo_open(rg_t *rg)
{
	int_fast32_t status = rg_get_curr_status(rg);
	rg_ctrl_t *ctrl = rg_get_new_ctrl(rg);
	...
	if (ctrl != NULL) {
		...
		if ((ctrl->ctrler == RG_CTRLER_CENTER)
			 ...
			 */
			if ((ctrl->action == RG_ACTION_UP)
				|| (ctrl->action == RG_ACTION_DN)
...
static int_fast32_t front_center_open(rg_t *rg)
{
...
static int_fast32_t front_center_close(rg_t *rg)
{
	return STATUS_IDLE;
}

static int_fast32_t front_center_stop(rg_t *rg)
{
	return STATUS_IDLE;
}

static int_fast32_t front_center_nc(rg_t *rg)
{
	int_fast32_t status = rg_get_curr_status(rg);
	rg_ctrl_t *ctrl = rg_get_new_ctrl(rg);
	...
	if (ctrl->ctrler == RG_CTRLER_LO) {
		...
		static const ctrl_handle_t tab[RG_ACTION_END] = {
			[RG_ACTION_UP] = {front_do_dummy, STATUS_CENTER_NC_LO_WAIT}
		};
 ...
 * @return int_fast32_t
 */
static int_fast32_t front_center_nc_lo_wait(rg_t *rg)
{
	int_fast32_t status = rg->status;
	rg_ctrl_t *ctrl = rg_get_new_ctrl(rg);
	...
	if (ctrl->ctrler == RG_CTRLER_CENTER) {
		...
		static const ctrl_handle_t tab[RG_CTRL_END][RG_ACTION_END] = {
			[RG_CTRL_MANUAL][RG_ACTION_NC_CLR] =
				{ front_set_up_signal, STATUS_LO_OPEN },
			[RG_CTRL_MANUAL][RG_ACTION_UP] =
				{ rg_set_up_signal, STATUS_CENTER_NC_CENTER_OPEN },
			[RG_CTRL_MANUAL][RG_ACTION_DN] =
				{ rg_down_s, STATUS_CENTER_NC_CENTER_CLOSE },
...
static int_fast32_t front_center_nc_center_open(rg_t *rg)
{
	int_fast32_t status = rg->status;
...
static int_fast32_t front_center_nc_center_close(rg_t *rg)
{
	return ((rg->prev_status == STATUS_CENTER_NC_LO_WAIT) ?
			STATUS_CENTER_NC_LO_WAIT : STATUS_CENTER_NC);
}

static int_fast32_t front_center_nc_center_stop(rg_t *rg)
{
	return ((rg->prev_status == STATUS_CENTER_NC_LO_WAIT) ?
			STATUS_CENTER_NC_LO_WAIT : STATUS_CENTER_NC);
}

static int_fast32_t front_center_no(rg_t *rg)
{
	int_fast32_t status = rg->status;
	rg_ctrl_t *ctrl = rg_get_new_ctrl(rg);
...
int_fast32_t mix_front_entrance(rg_t *rg)
{
	static const rg_sm_fp status_handle[STATUS_END] = {
		[STATUS_IDLE] = front_idle,
		[STATUS_LO_OPEN] = front_lo_open,
		[STATUS_CENTER_OPEN] = front_center_open,
		[STATUS_CENTER_CLOSE] = front_center_close,
		[STATUS_CENTER_STOP] = front_center_stop,
		[STATUS_CENTER_NC] = front_center_nc,
		[STATUS_CENTER_NC_LO_WAIT] = front_center_nc_lo_wait,
		[STATUS_CENTER_NC_CENTER_OPEN] = front_center_nc_center_open,
		[STATUS_CENTER_NC_CENTER_CLOSE] = front_center_nc_center_close,
		[STATUS_CENTER_NC_CENTER_STOP] = front_center_nc_center_stop,
		[STATUS_CENTER_NO] = front_center_no
	};
	...
	int_fast32_t next_status = status_handle[rg->status](rg);
