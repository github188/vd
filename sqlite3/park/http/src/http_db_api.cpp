...
sqlite3 *http_db = NULL;

int http_db_open(const char *path)
{
	int ret = sqlite3_open(path, &http_db);
	if(ret != SQLITE_OK)
		ERROR("Create park database failed:%s!", sqlite3_errmsg(http_db));
...
int http_db_insert_picreget_tb(sqlite3 *db, http_picreget_tb picreget_tb)
{
	...
	const char* sql = "insert into picreget(id, picture_path, picture_name, picture_size) values(?, ?, ?, ?);";
...
int http_db_delete_picreget_tb(sqlite3 *db, const char* pic_name)
{
	...
	sprintf(sql,"delete from picreget where picture_name=\"%s\";", pic_name);
...
int http_db_count_msgreget_callback(void *data, int ncols, char **values, char **headers)
{
...
int http_db_count_msgreget_tb(sqlite3 *db)
{
	...
	const char*sql = "select count(*) from msgreget;";
   	...
   	ret = sqlite3_exec(db, sql, http_db_count_msgreget_callback, (void *)&li_nrow, &perrmsg);
	if(ret != SQLITE_OK) {
		ERROR("%s", perrmsg);
	}
...
int http_db_count_picreget_callback(void *data, int ncols, char **values, char **headers)
{
	*(int *)data = atoi(values[0]);
...
int http_db_count_picreget_tb(sqlite3 *db)
{
	...
	const char* sql = "select count(*) from picreget;";
   	...
   	ret = sqlite3_exec(db, sql, http_db_count_picreget_callback, (void *)&li_nrow, &perrmsg);
	if(ret != SQLITE_OK) {
		ERROR("%s", perrmsg);
	}
